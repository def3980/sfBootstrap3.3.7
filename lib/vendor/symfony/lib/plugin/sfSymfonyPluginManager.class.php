 ...
 * Viernes, 21 Octubre 2016 19:65:21
 * + ------------------------------------------------------------------- +
 */
...
class sfSymfonyPluginManager extends sfPluginManager {
     ...
     *
     * Este es un metodo estatico que no se basa en el ambiente PEAR. por el cual
     * no queremos tener dependencia de PEAR.
     *
     * @param string $plugin    El nombre del plugin plugin
     * @param string $configDir El directorio de configuracion
     */
    static public function enablePlugin($plugin, $configDir) {
        if (!$configDir) {
            throw new sfPluginException('Debe proveer una opcion "config_dir".');
        }
