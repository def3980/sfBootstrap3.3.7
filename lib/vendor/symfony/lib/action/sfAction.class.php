 ...
 * Miércoles, 20 Enero 2016 22:06:22
 * + ------------------------------------------------------------------- +
 */
...
abstract class sfAction extends sfComponent {
    ...
    public function initialize($context, $moduleName, $actionName) {
        ...
        if ($file = $context
                        ->getConfigCache()
                        ->checkConfig(
                            'modules/'.$this->getModuleName().'/config/security.yml',
                            true
                        )) {
            require($file);
        }
    ...
    /**
     * Execute an application defined process immediately after execution 
     * of this sfAction object.
     *
     * By default, this method is empty.
     */
    ...
    /**
     * Forwards current action to the default 404 error action unless 
    ...
    /**
     * Redirects current action to the default 404 error action
     * (with browser redirection).
     *
     ...
     *
     * This method stops the action. So, no code is executed after
    ...
    /**
     * If the condition is true, forwards current action to a new one
    ...
    /**
     * Unless the condition is true, forwards current action to a new one
     * (without browser redirection).
     *
     * This method stops the action. So, no code is executed after
     * a call to this method.
    ...
    /**
     * Redirects current request to a new URL, only
    ...
    /**
     * Redirects current request to a new URL, unless specified condition
     * is true.
     *
     * This method stops the action. So, no code is executed after
     * a call to this method.
    ...
    /**
     * Appends the given text to the response content and bypasses
     * the built-in view system.
     *
    ...
    public function renderComponent($moduleName, $componentName, $vars = null) {
        return $this->renderText($this->getComponent(
                    $moduleName,
                    $componentName,
                    $vars
                ));
        ...
        if (sfConfig::get('sf_logging_enabled')) {
            $this->dispatcher->notify(
                new sfEvent(
                    $this,
                    'application.log',
                    array(sprintf('Change layout to "%s"', $name))
                )
            );
        }
    ...
    /**
     * Changes the default view class used for rendering the template
    ...
    public function setViewClass($class) {
        sfConfig::set(
            'mod_'.strtolower($this->getModuleName())
            .'_view_class',
            $class
        );
    }
    ...
    protected function get404Message($message = null) {
        return null === $message
                ? sprintf(
                        'This request has been forwarded to a 404 error page '
                        . 'by the action "%s/%s".',
                        $this->getModuleName(),
                        $this->getActionName()
                    )
