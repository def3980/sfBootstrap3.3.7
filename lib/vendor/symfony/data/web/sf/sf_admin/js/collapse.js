/**
 * Por Oswaldo Rojas
 * Empezado a modificar un Miércoles, 20 Enero 2016 07:22:24
 * Terminado un Miércoles, 20 Enero 2016 19:33:29
 */
...
var CollapsedFieldsets = {
    ...
    init: function() {
        ...
        for (var i = 0, fs; fs = fieldsets[i]; i++) {
            ...
            // of error messages.)
            if (fs.className.match(CollapsedFieldsets.collapse_re)
                && !CollapsedFieldsets.fieldset_has_errors(fs)) {
                collapsed_seen = true;
                ...
                collapse_link.onclick = new Function(
                    'CollapsedFieldsets.show(' + i + '); return false;'
                );
                collapse_link.href = '#';
        ...
        if (collapsed_seen) {
            ...
            Event.observe(
                findForm(document.getElementsByTagName('fieldset')[0]),
                'submit',
                function() {
    ...
    show: function(fieldset_index) {
        ...
        collapse_link.onclick = new Function(
            'CollapsedFieldsets.hide(' + fieldset_index + '); return false;'
        );
    ...
    hide: function(fieldset_index) {
        ...
        var collapse_link = document.getElementById('fieldsetcollapser' + fieldset_index);
        collapse_link.onclick = new Function(
            'CollapsedFieldsets.show(' + fieldset_index + '); return false;'
        );
        collapse_link.innerHTML = 'show';
    },
